...
public:
    ...
    float zFar;
    float aspectRatio;
    CAMERA(glm::vec3 position, glm::vec3 direction, float aspectRatio , float fov,float zNear,float zFar );
...
{
    shader -> setProj(fov , aspectRatio , zNear, zFar);
    // std::cout << aspectRatio << std::endl;
    shader -> setView(getPosition() , getDirection() );
}
CAMERA::CAMERA(glm::vec3 position, glm::vec3 direction , float aspectRatio ,float fov = 45 ,float zNear = 0.1 ,float zFar = 100 )
{
